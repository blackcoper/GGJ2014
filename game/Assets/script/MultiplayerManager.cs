...
public class MultiplayerManager : MonoBehaviour {
	...
	void follow_ally(){
		...
		if(ally_player.Length>0){
			...
			camera_follow camfollow = mainCamera.GetComponent<camera_follow>();
			camfollow.FocusTarget = ally_player[0].transform;
	...
	void spawn_player(){
		...
		Camera mainCamera =	GameObject.FindGameObjectWithTag("MainCamera").GetComponent<Camera>();
		camera_follow camfollow = mainCamera.GetComponent<camera_follow>();
		camfollow.FocusTarget = player.transform.FindChild("Model").transform;
